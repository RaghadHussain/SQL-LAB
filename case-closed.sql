...
WHERE language ILIKE 'Arabic' AND governmentform ILIKE 'Emirate Federation';
...
-- Write SQL query here
SELECT * FROM cities 
WHERE countrycode = 'ARE' AND name ILIKE 'Aj%';
...
countries.region,
cities.countrycode FROM countries 
JOIN cities ON cities.countrycode = countries.code 
WHERE cities.name ILIKE 'Sh%' AND region ILIKE 'Middle East';
...
WHERE countries.name = 'United Arab Emirates';
